...
// Serve the sibling `data-gmzz` / `resource-gmzz` artifact repos in dev.
// Walk ancestor directories until the sibling repo is found, so this keeps
// working from a git worktree. Override with GMZZ_DATA_DIR / GMZZ_RES_DIR.
...
const DATA_DIR = process.env.GMZZ_DATA_DIR ?? siblingRepo('data-gmzz')
const RES_DIR = process.env.GMZZ_RES_DIR ?? siblingRepo('resource-gmzz')
...
export default defineConfig({
  server: { host: '0.0.0.0', port: 15179, strictPort: true, allowedHosts: true },
  plugins: [
    arkiveFontAssets(),
    react(),
    tailwindcss(),
    staticDirPlugin('gmzz-data', '/data', DATA_DIR),
    staticDirPlugin('gmzz-res', '/gmzzres', RES_DIR),
  ],
