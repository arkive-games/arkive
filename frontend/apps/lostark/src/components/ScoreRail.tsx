  ...
  return (
    <aside className="sticky top-4 space-y-3 max-lg:static">
      ...
      <div className="rounded-xl border border-border bg-card/70 px-4 py-3 backdrop-blur">
        <h2 className="mb-2 text-sm font-medium text-muted-foreground">战力构成</h2>
...
function Component({ component }: { component: Result['components'][number] }) {
  ...
  return (
    <section className="mt-3 border-t border-line pt-2">
      <h3 className="mb-1 text-sm font-medium text-muted-foreground">{component.label}</h3>
      <dl className="space-y-1 text-sm">
        <Row label="基础" value={fmt(component.base)} />
        <Row label="总增幅" value={`×${total.toFixed(4)}`} />
      </dl>
      {active.length === 0 ? (
        <p className="mt-1 text-sm text-muted-foreground">暂无增幅</p>
      ) : (
        <ul className="mt-1 space-y-1">
          {active.map((a) => (
            <li key={a.name} className="flex items-center gap-2 text-sm">
              <span className="w-20 shrink-0 truncate text-muted-foreground">{a.name}</span>
              <span
