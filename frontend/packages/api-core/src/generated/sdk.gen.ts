 ...
 * Hidden comments
 *
 * The counterpart to the hidden-post list, scoped to the games you moderate. Without it a hidden comment could be restored only by someone who already knew its id, which nothing hands out once it is hidden.
 */
export const listHiddenForumComments = <ThrowOnError extends boolean = false>(options?: Options<ListHiddenForumCommentsData, ThrowOnError>): RequestResult<ListHiddenForumCommentsResponses, ListHiddenForumCommentsErrors, ThrowOnError> => (options?.client ?? client).get<ListHiddenForumCommentsResponses, ListHiddenForumCommentsErrors, ThrowOnError>({
    responseType: 'json',
 ...
 *
 * The author only. Idempotent: a slot that carries nothing is already in the state the caller asked for.
 */
export const detachForumPostImage = <ThrowOnError extends boolean = false>(options: Options<DetachForumPostImageData, ThrowOnError>): RequestResult<DetachForumPostImageResponses, DetachForumPostImageErrors, ThrowOnError> => (options.client ?? client).delete<DetachForumPostImageResponses, DetachForumPostImageErrors, ThrowOnError>({
    responseType: 'json',
 ...
 *
 * The author only. Re-attaching at the same position replaces what is there, so correcting one slot needs no detach first. The image is always re-encoded, so metadata including EXIF location is discarded, and an animated GIF keeps only its first frame. Rate limited per account.
 */
export const attachForumPostImage = <ThrowOnError extends boolean = false>(options: Options<AttachForumPostImageData, ThrowOnError>): RequestResult<AttachForumPostImageResponses, AttachForumPostImageErrors, ThrowOnError> => (options.client ?? client).put<AttachForumPostImageResponses, AttachForumPostImageErrors, ThrowOnError>({
 ...
 *
 * Resolves either the permanent uid or a special uid, and returns only publicly visible fields. Permanent links should use the uid, since a special uid can be reassigned. Deactivated accounts are reported as not found, and so are accounts whose profileVisibility withholds them from the caller.
 */
 ...
 *
 * Newest first. Subject to the account's activityVisibility, so a withheld list answers 404 rather than 403 — a 403 would confirm the account exists and is withholding.
 */
 ...
 *
 * Newest first. Subject to the account's activityVisibility.
 */
