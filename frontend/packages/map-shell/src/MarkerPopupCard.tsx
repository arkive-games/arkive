    ...
    >
      <CardContent className="flex flex-col px-4 py-4">
        <div className="flex items-baseline gap-2">
          <span className="text-lg font-bold leading-snug text-[#3D3D3D] dark:text-white">
            {name}
          </span>
          {idLabel && <IdLabel value={idLabel} className="px-1.5 py-0.5 text-xs" />}
        </div>
        {metaLine ? (
        ...
        {description ? (
          <div className="text-sm leading-relaxed whitespace-pre-line text-[#3D3D3D] dark:text-white">
            {description}
          </div>
        ) : (
