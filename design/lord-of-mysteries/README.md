...
- Selecting the current station type at every step
...
The forecast engine uses exhaustive sequence enumeration. A `most` hint requires that type to appear strictly more often than the other two types in the three-station window. An `equal` hint requires one station of each type.
