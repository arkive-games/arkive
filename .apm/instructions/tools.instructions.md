...
- **Historical, Y up:** `flip_y=True` (`py' = pixelHeight - py`). The 1A calibration matched this
  space, because the frontend rendered with Leaflet's `CRS.Simple`, whose latitude counts upward.
  That discrepancy is now GONE from both ends: Leaflet is retired, `map-engine-gl` renders in
  image space with y DOWN, and the extractors emit `flip_y=False`. Expect no flip anywhere; a
  `flip_y=True` in new code is a bug rather than a convention.
