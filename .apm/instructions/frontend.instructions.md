...
- **`hm.js` only counts the page it loads on**, so an SPA reports one pageview per visit unless
  ...
  `popstate` handler; a hash-routed app (meta) reports the **adopted route** and must pass the
  path explicitly, because the shell's default reads `location.hash` — which includes in-page
  scroll anchors that are not pages, and would file one view under several URLs. Repeats and the
  entry page are deduped inside the shell, so an extra call is harmless and a missing one is
  invisible.
...
`.github/workflows/deploy.yml` builds `meta`, `aion2`, `palworld`, `vrising`, `ro3` and `gmzz` on
a push touching `frontend/**`, then uploads each `dist/` with `edgeone makers deploy -n <project>`.
`sts2` and `lostark` are still built and linted by CI but not deployed — they have no project.
