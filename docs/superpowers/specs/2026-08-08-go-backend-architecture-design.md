...
Accepted cost: shared types such as `User` are duplicated across specs, because `$ref`s
into a shared components document break most TS generators. The structural types stay
compatible, so call sites are unaffected.

**The transport is axios, reversing what this document originally specified** (2026-08-13,
at the project owner's direction, when `api-core` was actually built). The original text
argued for fetch on two grounds, and both deserve to be recorded rather than quietly
dropped: roughly 13 KB gzipped saved on a tile-heavy site, and hey-api's middleware hooks
covering the interceptors that were thought to be axios's only real advantage. The first
still holds and was accepted as the price. The second turned out to understate what the
interceptors are for here — they carry the envelope unwrapping and the credential decision
for all 33 operations, so that behaviour lives in one place instead of at every call site.
Anyone reopening this should read `frontend/packages/api-core/src/client.ts`, where the two
non-obvious constraints are documented: `throwOnError: true` is mandatory because the
generated client otherwise *returns* rejections rather than raising them, and a conditional
`Payload<T>` type is what keeps the static types honest about what the interceptor already
unwrapped.
