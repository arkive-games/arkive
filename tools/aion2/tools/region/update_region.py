...
BORDER_FILENAME = "World_D_A_regions.json"

# BASE_URL = "http://localhost:9000/api/v1/maps/World_L_A"
# FILENAME = "region_light.json"
# BORDER_FILENAME = "World_L_A_regions.json"
...
async def update_region_borders():
    with open(BORDER_FILENAME, "r", encoding="utf-8") as f:
        regions = json.load(f)
        async with httpx.AsyncClient(headers=HEADERS) as client:
            for region in regions:
                # ---- 1) POST create region ----
                patch_payload = {"borders": region["polygons"]}
                patch_url = REGIONS_URL + f"/{region["region"]}"

                patch_resp = await client.patch(patch_url, json=patch_payload)
                print(f"[PATCH] {region["region"]} borders: {patch_resp.status_code}")
...
if __name__ == "__main__":
    asyncio.run(update_region_borders())
