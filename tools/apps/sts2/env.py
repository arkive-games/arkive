"""Per-machine paths for the STS2 pipelines — environment only, no defaults.

Set the variables in ``tools/.env`` (see ``tools/.env.example``) or export them:

  STS2_RAW       gdex export root (models.json + res/ tree)
  STS2_DATA_OUT  data-sts2 repo (dataset the frontend fetches)
  STS2_RES_OUT   resource-sts2 repo (WebP card art + icons)
"""
