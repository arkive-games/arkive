...
async def init_redis(backend_app: fastapi.FastAPI) -> None:
    ...
    logger.info(f"Connected to Redis at {settings.REDIS_HOST}:{settings.REDIS_PORT}/{settings.REDIS_DATABASE}")
...
def get_redis_client(backend_app: fastapi.FastAPI = Depends()) -> aioredis.Redis:
