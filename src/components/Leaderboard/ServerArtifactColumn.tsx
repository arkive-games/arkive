...
interface ServerArtifactColumnProps {
  ...
  formatCountdown: (recordTimeStr: string | undefined) => string;
  t: any;
  icons: {
    neutral: string;
    light: string;
  ...
  return (
    <div className="flex-1 flex flex-col items-center">
      ...
      <div className="flex flex-col items-center gap-2 mt-2">
        ...
        {/* Contributor Line */}
        {artifacts.length > 0 && artifactStateMap[matching.id]?.[artifacts[0].id]?.contributors?.[0] && (
          <div className="flex items-center justify-between w-full  mt-1" onClick={(e) => e.stopPropagation()}>
            <span className="text-[12px] text-default-800">
              <Trans
                t={t}
                i18nKey="common:leaderboard.providedBy"
                values={{ username: artifactStateMap[matching.id][artifacts[0].id].contributors[0].user.name }}
                components={{
